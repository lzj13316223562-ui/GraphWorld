#!/usr/bin/env python3
"""Plot no-robot score curves for fixed/calendar/stochastic schedules."""
...
def latest_run(scene: str, humans: int, schedule_mode: str) -> Path:
    ...
    for summary_path in (EXP_ROOT / scene).glob("steps_1600__robots_0__humans_*__model_npc_only_baseline*/**/summary.json"):
        try:
        ...
        if int(summary.get("steps", -1)) != 1600:
            continue
        if int(summary.get("robots", -1)) != 0 or int(summary.get("humans", -1)) != humans:
            continue
        if str(summary.get("schedule_mode") or "fixed") != schedule_mode:
            continue
        run_dir = summary_path.parent
        metrics_path = run_dir / "no_robot" / "metrics.csv"
        if metrics_path.exists():
            candidates.append((summary_path.stat().st_mtime, run_dir))
    if not candidates:
        raise FileNotFoundError(f"missing no_robot run: scene={scene} humans={humans} schedule={schedule_mode}")
    return max(candidates, key=lambda item: item[0])[1]


def load_rows(run_dir: Path) -> list[dict[str, float]]:
    path = run_dir / "no_robot" / "metrics.csv"
...
def main() -> None:
    ...
    plot_grid(
        data,
        AVG_METRICS,
        "No-Robot Schedule Perturbation: Cumulative Average Scores",
        "no_robot_schedule_avg_grid.png",
    )
    plot_grid(
        data,
        INSTANT_METRICS,
        "No-Robot Schedule Perturbation: Instant Scores",
        "no_robot_schedule_instant_grid.png",
    )
