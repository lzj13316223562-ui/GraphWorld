...
def latest_metrics(base_scene: str, humans: int, profile: str) -> Path:
    ...
    if not paths:
        raise FileNotFoundError(f"missing {base_scene} {profile}")
...
def main() -> None:
    ...
    for scene, _label, humans in SCENES:
        for profile, profile_label, _color in PROFILES:
            rows = read_rows(latest_metrics(scene, humans, profile))
            data[(scene, profile)] = rows
            last = rows[-1]
            summary_rows.append(
                {
                    "scene": scene,
    ...
    with (FIG_DIR / "profile_diversity_800_summary.csv").open("w", newline="", encoding="utf-8") as handle:
        fieldnames = ["scene", "profile", "final_score", "state_score", "spatial_score", "human_event_score"]
    ...
    for col, (scene, scene_label, _humans) in enumerate(SCENES):
        for row_idx, (metric, metric_label) in enumerate(METRICS):
            ...
            for profile, profile_label, color in PROFILES:
                rows = data[(scene, profile)]
                xs = [row["step"] for row in rows]
                ys = [row[metric] for row in rows]
                y_values.extend(ys)
                line = ax.plot(xs, ys, color=color, linewidth=1.45, label=profile_label)[0]
    ...
    fig.suptitle("Graph Profile Diversity: Qwen3.5-9B Goal-Review", fontsize=13, y=0.995)
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 0.965), ncol=3, frameon=False, fontsize=8.5)
